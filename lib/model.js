...
/**
 * Check if model is either Backbone Model or Collection (like)
 * @param  {Any}  model object to be checked
 * @returns {Boolean} true if it
 */
function isBackbone(model) {
  if (!model) return false;
  var isModel = !!(model.cid && model.attributes);
  var isCollection = _.isArray(model.models) && _.isObject(model._byId);
  return isModel || isCollection;
}
...
function getRelationValues(relation, modelAttributes) {
  ...
  _.find(relation.references, function(ref, key) {
    if (modelAttributes[ref])  {
      attrs[key] = modelAttributes[ref];
      all = true;
    } else {
      all = false;
      return true;
...
function getReferences(property, name, schema) {
  ...
  var _Model = property.model;
  var _Collection = property.collection;
  if (!_Model && _Collection) {
    _Model = _Collection.prototype.model;
    type = _Collection.prototype.type;
  }
  if (!_Model) {
    return;
  }
  type = _Model.prototype.type || type || name;
  srcAttr = _Model.prototype.idAttribute;
  if (!type || !srcAttr) {
    return;
  }
  dstAttr = type + '_id';
...
var Model = Backbone.Model.extend({
  ...
  get: function(attr, options) {
    if (this.virtualProperties) {
      if (_.isFunction(this.virtualProperties[attr])) {
        return this.virtualProperties[attr].call(this);
      }
      if (_.isObject(this.virtualProperties[attr])
        && _.isFunction(this.virtualProperties[attr].get)) {
        return this.virtualProperties[attr].get.call(this);
      }
    }
    var relation;
    if (this.relationDefinitions && (relation = this.relationDefinitions[attr])) {
      if (!this._prepareRelation(relation, attr, this.attributes, _.extend({}, options))) {
        return undefined;
      }
    }
  ...
  set: function(key, value, options) {
    ...
    if (this.virtualProperties) {
      _.each(attributes, function(attrValue, attrKey) {
        if (_.isObject(this.virtualProperties[attrKey])
          && _.isFunction(this.virtualProperties[attrKey].set)) {
  ...
  toJSON: function(options) {
    ...
    function convertProperty(properties, name) {
      ...
      if (projection) {
        if (projection.onlyFields && projection.onlyFields.indexOf(name) === -1) {
          return;
        }
        if (projection.removeFields && projection.removeFields.indexOf(name) > -1) {
          return;
        }
      }

      var attribute = this.get(name, options);
    ...
    if (this.schema && this.schema.properties) {
      ...
      // It may be boolean (true means to include all props) or array
      // specifying which properties to include
      if (options.includeVirtualProperties) {
        var filter = _.isArray(options.includeVirtualProperties)
          ? options.includeVirtualProperties
          : false;
  ...
  _sanitizeAttribute: function (attribute) {
  ...
  _convertAttributes: function(attributes) {
    _.each(attributes, function(attribute, name) {
      ...
      if (!attrDefinition) {
        ...
        if (!isFinite(date)) {
          // if date was invalid (cannot be parsed), don't convert it
  ...
  _prepareRelation: function(relation, name, attributes, options) {
    var self = this;
