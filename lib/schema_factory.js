 ...
 *         Schema definition or the schema id of a previously registered schema
 * @param  {Object} baseClass
 *         Provides an optional model or collection which overrides the default base class.
 * @returns {Object} Returns the contructed model or collection
 */
SchemaFactory.prototype.create = function(schema, baseClass) {
  ...
  delete this.parsedSchemaCache[schema.id];
...
SchemaFactory.prototype.expandReferences = function(schema, rootSchema, propertyName) {
  ...
  if (reference) {
    ...
    var fetchedSchema = this.parsedSchemaCache[referencedSchemaId]
      || this.getSchemaById(referencedSchemaId);
    referencedSchema = _.extend(
      {},
...
SchemaFactory.prototype.getCollection = function(options, models, collectionOptions) {
  var schemaId = options.referencedSchemaId;
  var CollectionClass = this._getBaseCollectionForSchemaId(schemaId, options)
    || this.baseCollection;
  if (!CollectionClass) {
    // throw new Error('Cannot construct collection for ' + schemaId);
    return null;
  }
  var instance = new CollectionClass(models, collectionOptions);
  return instance;
};
