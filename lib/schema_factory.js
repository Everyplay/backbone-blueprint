...
SchemaFactory.prototype.addRelation = function(schema, options) {
  ...
  if (options.parentSchema.type === 'array') {
    schema.collection = this._createCollection(
      schema,
      options,
      this._getBaseCollectionForSchemaId(options.referencedSchemaId, options)
    );
  } else {
    schema.model = this._createModel(
      schema,
      options,
      this._getBaseModelForSchemaId(options.referencedSchemaId, options)
    );
  }
...
/**
 * _getBaseModelForSchemaId
 * Return the Base class for given schemaId and options. This is function could be overriden
 * if custom class is to be mapped to schema.
 */
